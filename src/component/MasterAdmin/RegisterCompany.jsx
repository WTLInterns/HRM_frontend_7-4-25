...
//     try {
//       // For a real application, here you would make an API call to save the company
...
const RegisterCompany = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        // Update existing company (remains unchanged)
        const companies = JSON.parse(localStorage.getItem('companies') || '[]');
        const updatedCompanies = companies.map(company => 
          company.id === formData.id ? formData : company
        );
        ...
        // Replace {id} with the actual id value from formData in the URL
        const apiUrl = `http://localhost:8282/masteradmin/addSubAdmin/${formData.id}`;
        ...
        formDataToSend.append("registercompanyname", formData.registerCompanyName); // fixed key
